...
public:
	...
	bool enqueue( T element, bool overwrite );	//add an element
...
template<typename T, int rawSize>
bool SimpleFIFO<T,rawSize>::enqueue( T element, bool overwrite = false ) {
	if ( !overwrite && count() >= rawSize ) { return false; }
	numberOfElements++;
	nextIn %= size;
	raw[nextIn] = element;
